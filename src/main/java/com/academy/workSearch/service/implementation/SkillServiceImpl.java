...
import com.academy.workSearch.exceptionHandling.NoSuchEntityException;
...
public class SkillServiceImpl  implements SkillService  {
    ...
    public SkillDetailsDTO update(SkillDetailsDTO skill) throws ValidationException {
        ...
        Skill skillInDB = Optional.ofNullable(skillDAO.get(id))
                .orElseThrow(() ->
        ...
        skillDAO.save(skillInDB);
        return SKILL_DETAILS_MAPPER.toDto(skillInDB);
    }

    public SkillDetailsDTO get(UUID id) {
        Skill skill = Optional.ofNullable(skillDAO.get(id))
                        .orElseThrow(() ->
                                new NoSuchEntityException(String.format(NO_SUCH_SKILL_FORMAT, NO_SUCH_SKILL + "id", id)));
    ...
    public SkillDetailsDTO getByName(String name) {
        Skill skill = Optional.ofNullable(skillDAO.getByName(name))
                .orElseThrow(() ->
                        new NoSuchEntityException(String.format(NO_SUCH_SKILL_FORMAT, NO_SUCH_SKILL + "name", name)));
        return SKILL_DETAILS_MAPPER.toDto(skill);
    }
