...
export class WorkerSkillsComponent implements OnInit, OnDestroy {
  ...
  showModalForm(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {      
      if (result.submit) {      
        const errMessageIncorrectSkillList = "You entered an incorrect list of skills!";  
        const skillNames: string[] = result.skills
                                          .split(';')
                                          .map(skillName => skillName.trim())
                                          .filter(skillName => skillName!=="");
        skillNames.length>0? 
          this.addSkills(skillNames) : this.toastr.warning(errMessageIncorrectSkillList, 'Warning!'); 
      }
    });
  }
